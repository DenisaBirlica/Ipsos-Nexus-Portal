      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
        ...
        {showAddSpace && (
          <div style={{ marginBottom: '2rem', background: 'rgba(255, 255, 255, 0.1)', padding: '1.5rem', borderRadius: '16px' }}>
            <form onSubmit={handleAddSpace} style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
              <input
                type="text"
                placeholder="New Space Name"
                value={newSpaceName}
                onChange={(e) => setNewSpaceName(e.target.value)}
                style={{ flex: '1 1 200px', minWidth: '200px', padding: '0.75rem', borderRadius: '8px', border: 'none', background: 'rgba(255, 255, 255, 0.9)' }}
                required
              />
              <select 
                value={newSpaceType} 
                onChange={e => setNewSpaceType(e.target.value as SpaceType)}
                style={{ flex: '1 1 150px', minWidth: '150px', padding: '0.75rem', borderRadius: '8px', border: 'none', background: 'rgba(255, 255, 255, 0.9)' }}
              >
                        ...
                        <div style={{ display: 'flex', gap: '0.25rem' }}>
                            {deletingId === space.id ? (
                                <div style={{display: 'flex', alignItems: 'center', gap: '0.25rem'}}>
                                    <button onClick={() => confirmDelete(space.id)} style={{background:'none', border:'none', color:'#34D399', cursor: 'pointer', padding: '0.25rem'}}><CheckIcon width={20}/></button>
                                    <button onClick={() => setDeletingId(null)} style={{background:'none', border:'none', color:'#F87171', cursor: 'pointer', padding: '0.25rem'}}><XMarkIcon width={20}/></button>
                                </div>
                            ) : (
                                <>
                                    <button 
                                    ...
                                    <button onClick={() => setDeletingId(space.id)} style={{background:'none', border: 'none', color:'white', padding:'0.25rem', cursor: 'pointer'}}><TrashIcon width={16}/></button>
                                </>
                            )}
                        </div>
                    </div>
